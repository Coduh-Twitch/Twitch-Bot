  ...
  if (req.query.code) {
    ...
    try {
      ...
      if (tokenRes.data) {
        ...
        try {
          let currentUser: { data: any } = await get(
          ...
          if (currentUser.data?.data?.[0]?.id) {
            ...
            res.send({ ...(sessionData || null), ...tokenRes.data });
          }
        } catch (e) {
          console.log(e);
        }
    ...
    res.redirect(
      `https://id.twitch.tv/oauth2/authorize?client_id=${process.env.CLIENT_ID}&redirect_uri=${process.env.APP_URL}/auth&scope=${scopes.join(" ")}&response_type=code&force_verify=true`,
    );
  }
});
