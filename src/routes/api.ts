...
import { apiClient, broadcasterApiClient, CHANNEL, client, commandsMap, lastFetchedClipId, reply, setFetchedClipId } from "..";
...
apiRouter.get("/clips/chosen", async (req, res) => {
    ...
    let sessionRes = await sessionModel.findOne({userId: process.env.BOT_USER_ID});
    ...
    if(clip.id !== "ClipNotFound") {
        let downloadInfo = await axios.get(`https://api.twitch.tv/helix/clips/downloads?editor_id=${sessionRes.userId}&broadcaster_id=${process.env.CHANNEL_ID}&clip_id=${clip.id}`, {headers})
        ...
        clip.portrait_download_url = (downloadInfo as any).portrait_download_url || null;
