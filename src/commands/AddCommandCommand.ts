...
const AddCommandCommand: ChatCommand = {
  enabled: true,
  name: "addcommand",
  aliases: ["addcmd", "ac", "addcommand", "addcom"],
  ...
  args: [
    ...
    },
    {
      name: "response",
      description:
        "What the bot should respond with when this command is triggered",
      required: true,
    },
  ],
  run: async (client, user, content, message) => {
    ...
    if (!cmd)
      return reply(
        client,
        user,
        `Please provide a command trigger and response (!addcommand <trigger> <response>)`,
      );
    ...
    try {
      ...
      if (alreadyExists)
        return reply(
          client,
          user,
          `Command "${alreadyExists.trigger}" already exists`,
        );
      ...
      reply(
        client,
        user,
        `Successfully Added Command ${doc.trigger} -> ${doc.content}`,
      );
    } catch (e) {
      reply(
        client,
        user,
        `Failed while trying to add command. Please try again.`,
      );
    }
  },
};
