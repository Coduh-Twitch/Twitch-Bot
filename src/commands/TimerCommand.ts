...
const TimerCommand: ChatCommand = {
    ...
    run: async (client, user, content, message) => {
        ...
        let subcommandData = TimerCommand.subCommands.find(sc => sc.name.toLowerCase() === subcommand);
        ...
        if (!authority || !subcommand || !subcommandData) {
            ...
            // !timer?
            let timer = getTimer();
            let formattedDuration = moment.duration(timer.seconds, "seconds").format(`${timer.seconds >= 86400 ? "DD:" : ""}${timer.seconds >= 3600 ? "HH:" : ""}${timer.seconds >= 60 ? "mm:" : "[00:]"}ss`);

            await reply(client, user, `⏱️ ${timer.paused ? `The timer is paused!${timer.seconds === 0 ? "" : ` There is ${formattedDuration} remaining.`}` : timer.seconds === 0 ? "The timer is not running!" : `The timer has ${formattedDuration} remaining!`}`, message)
            ...
            if (subcommand === "set") {
                try {
                    ...
                    if (!duration || duration_seconds === 0) return reply(client, user, `Correct Usage: !${TimerCommand.name} ${subcommand} {duration} (i.e. !${TimerCommand.name} ${subcommand} 20m)`, message);
                    if (duration_seconds >= 2505600) return reply(client, user, `Duration must be less than 29 days`, message);
                    ...
                    reply(client, user, `⏱️ Set the timer to ${formattedDuration}`, message)
                } catch (e) {
                    return reply(client, user, `Correct Usage: !${TimerCommand.name} ${subcommand} {duration} (i.e. !${TimerCommand.name} ${subcommand} 20m)`, message);
                }
            }

            if (subcommand === "show") {
                let timer = getTimer();
                if (timer.visible) return reply(client, user, `The timer is already visible!`, message);
                try {
                    setTimerVisibility(true);
                    reply(client, user, `Showing the timer!`, message)
                } catch (e) {
                    return reply(client, user, `Failed to change timer visibility`, message)
                }
            }

            if (subcommand === "hide") {
                let timer = getTimer();
                if (!timer.visible) return reply(client, user, `The timer is already hidden!`, message);
                try {
            ...
            if (subcommand === "pause") {
                let timer = getTimer();
                if (timer.paused) return reply(client, user, `The timer is already paused!`, message);
                try {
                    setTimerPaused(true);
            ...
            if (subcommand === "unpause") {
                let timer = getTimer();
                if (!timer.paused) return reply(client, user, `The timer is not paused!`, message);
                try {
