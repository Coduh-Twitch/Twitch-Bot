  ...
  run: async (client, user, content, message) => {
    if (message.userInfo.isMod || message.userInfo.isBroadcaster)
      return await reply(
        client,
        user,
        `NotLikeThis Moderators can't do that`,
        message,
      );
    ...
    if (nukers.has(userId)) {
      return await reply(
        client,
        user,
        `NotLikeThis Wait a moment before nuking yourself again. Do you need help?`,
        message,
      );
    }
    ...
    await apiClient.asUser(process.env.BOT_USER_ID, async (ctx) => {
      ctx.moderation
        .banUser(process.env.CHANNEL_ID, {
          user: message.userInfo.userId,
        ...
        .catch(async (e) => {
          ...
          await reply(
            client,
            user,
            `DinoDance @${user} tried to nuke themselves, but it didn't go as planned... 🧨`,
            message,
          );
        });
    });
