import { apiClient, reply } from "../index";
...
const RoomCodeCommand: ChatCommand = {
  ...
  run: async (client, user, content, message) => {
    ...
    if (!stream)
      return reply(client, user, `The stream is currently offline.`, message);
    ...
    if (!code)
      return reply(
        client,
        user,
        `The Room Code must be set by a Moderator first.`,
        message,
      );
    ...
    if (game.includes("Mario Kart World")) {
      return reply(
        client,
        user,
        `To join: Online -> Friends -> Enter Room Code -> ${code}`,
        message,
      );
      ...
      return reply(
        client,
        user,
        `To join: https://jackbox.tv -> Log in with Twitch -> Enter Code -> ${code}`,
        message,
      );
    } else {
