...
const EditCommandCommand: ChatCommand = {
  enabled: true,
  name: "editcommand",
  aliases: ["editcmd", "changecommand", "ec", "editcom"],
  ...
  args: [
    ...
    },
    {
      name: "new_response",
      description:
        "The new response of the edited custom command. -level=UserRoles for permission levels",
      required: true,
    },
  ],
  run: async (client, user, content, message) => {
    ...
    if (!cmd)
      return reply(
        client,
        user,
        `Please provide a command trigger and response (!editcommand <trigger> <new_response>)`,
      );
    ...
    try {
      if (args.startsWith("-level")) {
        ...
        if (userLevel > UserRoles.DEFAULT)
          return reply(client, user, `Invalid Permission Level`);

        const customCommand = await customCommandModel.findOneAndUpdate(
          { trigger: cmd },
          { userLevel: UserRolesStringMap[`${userLevel}`] },
        );
        if (!customCommand)
        ...
        reply(
          client,
          user,
          `Successfully Updated Command ${customCommand.trigger}`,
        );
      }
    } catch (e) {
      reply(
        client,
        user,
        `Failed while trying to update command. Please try again.`,
      );
    }
  },
};
