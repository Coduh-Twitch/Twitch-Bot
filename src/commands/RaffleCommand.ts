...
const RaffleCommand: ChatCommand = {
    ...
    run: async (client, user, content, message) => {
        ...
        if(Number.isNaN(Number(amount))) {
            if(amount.toString() === "cancel") {
                ...
                if(raffle) {
                    ...
                    reply(client, user, `GoldPLZ @${message.userInfo.userName} Cancelled the raffle for ${dr.points.toLocaleString()} point${dr.points === 1 ? "" : "s"} with ${parts.length} participant${parts.length === 1 ? "" : "s"}`);
        ...
        if(amount > 10000) return reply(client, user, `WTRuck Are you insane? ${amount.toLocaleString()} is way too many points. (max 10k)`, message)
