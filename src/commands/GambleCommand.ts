...
const GambleCommandCommand: ChatCommand = {
    ...
    run: async (client, user, content, message) => {
        ...
        if(!gamblePts || Number.isNaN(parseInt(gamblePts))) return reply(client, user, `Please provide an amount of points to roll (!roll <points>)`)
        ...
        if(points < gamblePts) return reply(client, user, points === 0 ? 'You have no points. You are broke. :(' : `You only have ${points} point${points === 1 ? "" : "s"} :(`)  
        if(gamblePts <= 0) return reply(client, user, points === 0 ? 'You have no points. You are broke. :(' : `You have to bet more than 0 points.`)
        ...
        try {
            ...
            if(winIndex === -1) {
                await userModel.findOneAndUpdate({twitchId: message.userInfo.userId}, {points: points - gamblePts})
                reply(client, user, `${slotStr} -> You lost ${gamblePts} point${gamblePts === 1 ? "" : "s"} :(`, message)
            } else {
                
                await userModel.findOneAndUpdate({twitchId: message.userInfo.userId}, {points: points + winPoints})
                reply(client, user, `${slotStr} -> ${jackpot ? "You won the jackpot! You" : "The house felt bad, so you"} got ${winPoints} point${winPoints === 1 ? "" : "s"}${jackpot ? "!" : " back before being thrown out the back door."}`, message)
            }
